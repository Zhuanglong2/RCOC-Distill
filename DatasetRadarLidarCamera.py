...
from utils import invert_pose_v2, rotate_forward, quaternion_from_matrix, invert_pose
...
def downsample_point_cloud(points, num_samples):
    ...
    参数:
        points (numpy.ndarray): 点云数组，形状为 (4, N)。
        num_samples (int): 采样点数。

    返回:
        numpy.ndarray: 下采样后的点云，形状为 (4, num_samples)。
    """
    if points.shape[1] <= num_samples:
        return points  # 如果点数少于采样数，不做处理
    indices = np.random.choice(points.shape[1], num_samples, replace=False)
    return points[:, indices]

class DatasetLidarCameraKittiOdometry(Dataset):

    def __init__(self, dataset_dir, dataset, transform=None, augmentation=False, use_reflectance=False,
                 max_t=1.5, max_r=20., split='val', device='cpu', val_sequence='00', suf='.png'):
        super(DatasetLidarCameraKittiOdometry, self).__init__()
        self.use_reflectance = use_reflectance  # False 不使用反射率
        self.maps_folder = ''
        self.device = device        
        self.max_r = max_r          # 20
        self.max_t = max_t          # 1.5
        ...
        self.all_files = []
        if self.dataset == 'NTU4DRadLM':
            if val_sequence == '06':
                self.sequence_list = ['00', '06']
            elif val_sequence == '07':
                self.sequence_list = ['01', '07']
            elif val_sequence == '08':
                self.sequence_list = ['02', '08']
        elif self.dataset == 'Dual_Radar':
            self.sequence_list = ['00', '01']  # dual_radar
        else:
        ...
        for seq in self.sequence_list:
            ...
            self.K[seq] = calib.K_cam2 # 3x3 camera intrinsic parameter

            T_cam02_velo_np2 = calib2.T_cam2_velo  # gt pose from cam02 to velo_lidar (T_cam02_velo: 4x4)
            self.K2[seq] = calib2.K_cam2  # 3x3 camera intrinsic parameter

            self.GTs_T_cam02_velo[seq] = T_cam02_velo_np #gt pose from cam02 to velo_lidar (T_cam02_velo: 4x4)
            self.GTs_T_cam02_velo2[seq] = T_cam02_velo_np2  # gt pose from cam02 to velo_lidar (T_cam02_velo: 4x4)

            image_list = os.listdir(os.path.join(dataset_dir, 'sequences', seq, 'image_2'))
            image_list.sort()

            for image_name in image_list:
                if not os.path.exists(os.path.join(dataset_dir, 'sequences', seq, 'velodyne', str(image_name.split('.')[0][-6:])+'.bin')):
                    continue
        ...
        if split == 'val' or split == 'test':
            ...
            val_RT_file = os.path.join(dataset_dir, 'sequences',
                                       f'val_RT_left_seq{val_sequence}_{max_r:.2f}_{max_t:.2f}.csv')    # 最大旋转20度,最大平移1.5m
    ...
    def safe_matmul(self, A, B):
        """安全执行矩阵乘法，处理NaN和无穷大"""
        # 检查并替换A中的无效值
        if np.isnan(A).any() or np.isinf(A).any():
            # print("警告: 矩阵A包含NaN或无穷大，已替换为0")
            A = np.nan_to_num(A)

        # 检查并替换B中的无效值
        if np.isnan(B).any() or np.isinf(B).any():
            # print("警告: 矩阵B包含NaN或无穷大，已替换为0")
            B = np.nan_to_num(B)
    ...
    def __getitem__(self, idx):
        item = self.all_files[idx]      # '00/000003'
        seq = str(item.split('/')[0])       # '00'
        rgb_name = str(item.split('/')[1])  # '000003'
        img_path = os.path.join(self.root_dir, 'sequences', seq, 'image_2', rgb_name+self.suf)
        lidar_path = os.path.join(self.root_dir, 'sequences', seq, 'velodyne', rgb_name[-6:]+'.bin')
        lidar_scan = np.fromfile(lidar_path, dtype=np.float32)

        lidar_path2 = os.path.join(self.root_dir, 'sequences', seq, 'velodyne-lidar', rgb_name[-6:] + '.bin')
        lidar_scan2 = np.fromfile(lidar_path2, dtype=np.float32)

        if self.dataset == 'NTU4DRadLM':
            # lidar  NTU
            pc2 = lidar_scan2.reshape((-1, 3))  # ntu-lidar
            num_point = pc2.shape[0]
            pc2 = np.hstack((pc2[:, 0:3], np.ones((num_point, 1))))

            # radar （NTU）
            a = lidar_scan.shape[0] % 3  # 防止不被3整除
            pc = lidar_scan[:lidar_scan.shape[0] - a].reshape((-1, 3))
        elif self.dataset == 'Dual_Radar':
            # lidar  dual-radar
            pc2 = lidar_scan2.reshape((-1, 6)) # dual-lidar
            pc2 = pc2[:, :4]

            # dual radar
            a = lidar_scan.shape[0] % 5  #防止不被3整除
            pc = lidar_scan[:lidar_scan.shape[0]-a].reshape((-1, 5))
            pc = pc[:, :3]
        else:
            raise TypeError('dataset is wrong!')

        valid_indices = pc[:, 0] < -3.      # 太近处的点云不要
        valid_indices = valid_indices | (pc[:, 0] > 3.)
        valid_indices = valid_indices | (pc[:, 1] < -3.)
        valid_indices = valid_indices | (pc[:, 1] > 3.)
        pc = pc[valid_indices].copy()
        valid_indices = pc2[:, 0] < -3.  # 太近处的点云不要
        valid_indices = valid_indices | (pc2[:, 0] > 3.)
        valid_indices = valid_indices | (pc2[:, 1] < -3.)
        valid_indices = valid_indices | (pc2[:, 1] > 3.)
        pc2 = pc2[valid_indices].copy()

        pc_org = torch.from_numpy(pc.astype(np.float32))    # (N, 4)
        pc_org2 = torch.from_numpy(pc2.astype(np.float32))  # (N, 4)

        if self.use_reflectance:
            reflectance = pc[:, 3].copy()
            reflectance = torch.from_numpy(reflectance).float()

        RT = self.GTs_T_cam02_velo[seq].astype(np.float32)   # (4, 4)
        RT2 = self.GTs_T_cam02_velo2[seq].astype(np.float32) # (4, 4)

        pc_vr = None
        pc_rcs = None
        if pc_org.shape[1] == 4 or pc_org.shape[1] == 3:
            pc_org = pc_org.t()     # (4, N)
        elif pc_org.shape[1] == 5:
            pc_vr = torch.cat((pc_org[:, :3].t(), pc_org[:, 3].unsqueeze(1).t()), 0)
            pc_rcs = torch.cat((pc_org[:, :3].t(), pc_org[:, 4].unsqueeze(1).t()), 0)

            pc_org = pc_org[:, :3].t()  # (3, N)
        ...
        if not torch.all(pc_org2[3, :] == 1.):
            pc_org2[3, :] = 1.


        pc_rot2 = self.safe_matmul(RT2, pc_org2.numpy())
        # pc_rot2 = np.matmul(RT2, pc_org2.numpy())
        pc_rot2 = pc_rot2.astype(np.float32).copy()
        pc_in2 = torch.from_numpy(pc_rot2)
        ...
        pc_vr_in = None
        pc_rcs_in = None
        if pc_vr is not None:
            pc_vr_rot = np.matmul(RT, pc_vr.numpy())  # (4, N)
            pc_vr_rot = pc_vr_rot.astype(np.float32).copy()
            pc_vr_in = torch.from_numpy(pc_vr_rot)

            pc_rcs_rot = np.matmul(RT, pc_rcs.numpy())  # (4, N)
            pc_rcs_rot = pc_rcs_rot.astype(np.float32).copy()
            pc_rcs_in = torch.from_numpy(pc_rcs_rot)

        h_mirror = False
        img = Image.open(img_path)

        if self.dataset == 'Dual_Radar':
            # dual-radar
            img = img.resize((960, 600))  # resize image with high-quality

        img_rotation = 0.
        try:
            img = self.custom_transform(img, img_rotation, h_mirror)    # 图片归一化标准化 (3, 376, 1241)
        except OSError:
        ...
        if self.split == 'train':
            ...
            pc_in2 = rotate_forward(pc_in2, R, T)

            if pc_vr_in is not None:
                R = mathutils.Euler((radians(img_rotation), 0, 0))
                T = mathutils.Vector((0., 0., 0.))
                pc_vr_in = rotate_forward(pc_vr_in, R, T)

                R = mathutils.Euler((radians(img_rotation), 0, 0))
                T = mathutils.Vector((0., 0., 0.))
                pc_rcs_in = rotate_forward(pc_rcs_in, R, T)
