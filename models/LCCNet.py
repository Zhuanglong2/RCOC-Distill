...
class LCCNet(nn.Module):
    ...
    def __init__(self, image_size, use_feat_from=1, md=4, use_reflectance=False, dropout=0.0,
        ...
        self.attention = attention  # False
        self.inplanes = 64
        if self.res_num == 50:
            layers = [3, 4, 6, 3]
        ...
        if self.attention:
            ...
            if self.res_num == 50:
                block = Bottleneck
            elif self.res_num == 18:    # resnet 18
                block = BasicBlock
    ...
    def forward(self, rgb, lidar):  # (1, 3, 256, 512)  (1, 1, 256, 512)
        H, W = rgb.shape[2:4]
        ...
        if self.pretrained_encoder:
            ...
            c23 = self.layer1_lidar(self.maxpool_lidar(c22))  # (1, 64, 64, 128)
            c24 = self.layer2_lidar(c23)  # 8       (1, 128, 32, 64)
            c25 = self.layer3_lidar(c24)  # 16      (1, 256, 16, 32)
            c26 = self.layer4_lidar(c25)  # 32      (1, 512, 8, 16)
        ...
        corr6 = self.corr(c16, c26)  # (1, 512, 8, 16)
        corr6 = self.leakyRELU(corr6)   # (1, 81, 8, 16)
        x = torch.cat((self.conv6_0(corr6), corr6), 1)  # (1, 128+81, 8, 16)
        x = torch.cat((self.conv6_1(x), x), 1)      # (1, 128+81+128, 8, 16)
        x = torch.cat((self.conv6_2(x), x), 1)
        x = torch.cat((self.conv6_3(x), x), 1)
        x = torch.cat((self.conv6_4(x), x), 1)      # (1, 529, 8, 16)
        ...
        x = x.view(x.shape[0], -1)      # (1, 529, 8, 16)
        x = self.dropout(x)
        x = self.leakyRELU(self.fc1(x))     # (1, 512)
...
if __name__ == '__main__':
    rgb_img = torch.randn(1, 3, 256, 512).to('cuda')
    refl_img = torch.randn(1, 1, 256, 512).to('cuda')
    ...
    model = LCCNet((256, 512), use_feat_from=feat, md=md,
    ...
    flops, params = profile(model, inputs=(rgb_img, refl_img,))
    ...
    # 测试参数设置
    calculate_fps(model,
                  inputs=(rgb_img, refl_img),  # 根据模型输入调整
