...
from losses2 import DistancePoints3D, GeometricLoss, L1Loss, ProposedLoss, CombinedLoss
from models.FourDRCOC import TestNet


from quaternion_distances import quaternion_distance

from tensorboardX import SummaryWriter
from utils2 import (mat2xyzrpy, merge_inputs, overlay_imgs, quat2mat,
...
# CCN training
@ex.capture
def train(model, optimizer, rgb_img, refl_img, lidar_input_gt, target_transl, target_rot, loss_fn, point_clouds, loss):
    model.train()

    optimizer.zero_grad()

    transl_err, rot_err, predict_depth_image = model(rgb_img, refl_img, False)

    if loss == 'points_distance' or loss == 'combined':
        losses = loss_fn(point_clouds, target_transl, target_rot, transl_err, rot_err, predict_depth_image, refl_img)
    else:
...
# CNN test
@ex.capture
def val(model, rgb_img, refl_img, lidar_input_gt,target_transl, target_rot, loss_fn, point_clouds, loss):
    model.eval()

    # Run model
    with torch.no_grad():
        transl_err, rot_err = model(rgb_img, refl_img, True)

    predict_depth_image = None
    if loss == 'points_distance' or loss == 'combined':
        losses = loss_fn(point_clouds, target_transl, target_rot, transl_err, rot_err, predict_depth_image, refl_img)
    else:
        losses = loss_fn(target_transl, target_rot, transl_err, rot_err)
...
def main(_config, _run, seed):
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, R_predicted,  T_predicted = train(model, optimizer, rgb_input, lidar_input,lidar_input_gt,
        ...
        for batch_idx, sample in enumerate(ValImgLoader):
            ...
            loss, trasl_e, rot_e, R_predicted,  T_predicted = val(model, rgb_input, lidar_input, lidar_input_gt,
                                                                  sample['tr_error'], sample['rot_error'],
                                                                  loss_fn, sample['point_cloud'], _config['loss'])
