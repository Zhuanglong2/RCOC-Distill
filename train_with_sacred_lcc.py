...
from models.CMRNet import CMRNet
...
def train(model, optimizer, rgb_img, refl_img, lidar_input_gt, target_transl, target_rot, loss_fn, point_clouds, loss):
    ...
    # Run model
    transl_err, rot_err = model(rgb_img, refl_img)  # (120, 3) (120, 4)

    predict_depth_image = None
    if loss == 'points_distance' or loss == 'combined':
        losses = loss_fn(point_clouds, target_transl, target_rot, transl_err, rot_err, None, None, lidar_input_gt)
    else:
...
def val(model, rgb_img, refl_img, lidar_input_gt,target_transl, target_rot, loss_fn, point_clouds, loss):
    ...
    # Run model
    with torch.no_grad():
        transl_err, rot_err = model(rgb_img, refl_img)      # (10, 3)   (10, 4)
...
def main(_config, _run, seed):
    ...
    if _config['network'].startswith('Res'):
        ...
        # model = CMRNet(input_size, use_feat_from=feat, md=md,
        #                use_reflectance=_config['use_reflectance'], dropout=_config['dropout'])
