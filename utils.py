...
def invert_pose_v2(R, T):
    T = np.matrix(T).T
    R = np.matrix(R.to_matrix())
    RT = np.matrix(np.eye(4))
    RT[:3, :3] = R
    RT[:3, 3] = T
    inv_RT = np.linalg.inv(RT)
    inv_R = inv_RT[:3, :3]
    temp_q = Rotation.from_matrix(inv_R).as_quat()
    inv_q = np.zeros(4)
    inv_q[0], inv_q[1], inv_q[2], inv_q[3] = temp_q[3], temp_q[0], temp_q[1], temp_q[2]
    temp_T = inv_RT[:3, 3]
    inv_T = np.zeros(3)
    inv_T[0], inv_T[1], inv_T[2] = temp_T[0, 0], temp_T[1, 0], temp_T[2, 0]
    return inv_q, inv_T
...
def merge_inputs(queries):
    ...
    returns = {key: default_collate([d[key] for d in queries]) for key in queries[0]
               if key != 'point_cloud' and key != 'point_cloud2' and key != 'rgb' and key != 'reflectance' and key != 'VR' and key != 'RCS'}
    for input in queries:
        point_clouds.append(input['point_cloud'])
        point_clouds2.append(input['point_cloud2'])
        VR.append(input['VR'])
        RCS.append(input['RCS'])
...
# rgb (3, 384, 1280) lidar (1, 1, 384, 1280)
def overlay_imgs(rgb, lidar, idx=0):
    std = [0.229, 0.224, 0.225]
    mean = [0.485, 0.456, 0.406]

    rgb = rgb.clone().cpu().permute(1,2,0).numpy()  # (384, 1280, 3)
    rgb = rgb*std+mean      # unnormalize
    lidar = lidar.clone()   # (1, 1, 384, 1280)

    lidar[lidar == 0] = 1000.
    lidar = -lidar
    #lidar = F.max_pool2d(lidar, 3, 1, 1)
    lidar = F.max_pool2d(lidar, 3, 1, 1)    # pool的kernel是3深度稍微平滑一下
    lidar = -lidar
    lidar[lidar == 1000.] = 0.

    #lidar = lidar.squeeze()
    lidar = lidar[0][0]     # (384, 1280)
    lidar = (lidar*255).int().cpu().numpy()
    lidar_color = cm.jet(lidar)
    lidar_color[:, :, 3] = 0.9      # alpha 透明度 0.5
    lidar_color[lidar == 0] = [0, 0, 0, 0]
    blended_img = lidar_color[:, :, :3] * (np.expand_dims(lidar_color[:, :, 3], 2)) + \
                  rgb * (1. - np.expand_dims(lidar_color[:, :, 3], 2))
    blended_img = blended_img.clip(min=0., max=1.)  # 混合两张图像并限制在0-1之间
